...
T DoublyLinkedList<T>::getItem(size_t position) const
{
  ...
  //changed
  if (pos_ptr == nullptr) {
    throw std::out_of_range("Error: item not found at the position " 
...
void DoublyLinkedList<T>::swap(int i, int j) {
    ...
    if (i < 0 || j >= item_count_) {
        return;
    }
    Node<T>* first_node = getPointerTo(i);
    Node<T>* second_node = getPointerTo(j);
    if (!first_node || !second_node) {
        return;
    }
    T temp = first_node->getItem();
    first_node->setItem(second_node->getItem());
    second_node->setItem(temp);
}